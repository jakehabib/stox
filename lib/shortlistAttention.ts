...
export const SHORTLIST_ATTENTION = {
  /**
   * Attention units a department produces in a week, before staff quality.
...
export function staffQuality(scouts: AttentionScout[]): number {
  const best = scouts.reduce((m, s) => Math.max(m, clamp(s.accuracy, 0, 100)), 0);
  const [lo, hi] = SHORTLIST_ATTENTION.STAFF_QUALITY_RANGE;
  return lo + (hi - lo) * (best / 100);
}
...
export function dynastyThroughput(mods: { attrBandMult: number; potBandMult: number }): number {
  const invested = clamp(((1 - mods.attrBandMult) + (1 - mods.potBandMult)) / 2, 0, 1);
...
export interface AttentionShare {
  ...
  name: string;
  position: string;
  /** Attention units this player actually received this week. The number the UI may quote. */
  units: number;
...
export interface AttentionSummary {
  teamId: string | null;
  shortlisted: number;
  /** Attention units per shortlisted player, after staff and Dynasty scaling. Every entry in `shares` was moved by exactly this. */
  unitsEach: number;
...
export async function applyShortlistAttention(
  ...
  for (const p of targets) {
    ...
    const close = clamp(
      unitsEach * SHORTLIST_ATTENTION.CLOSE_PER_UNIT * (matched ? SHORTLIST_ATTENTION.SPECIALTY_BONUS : 1),
      0,
      SHORTLIST_ATTENTION.MAX_WEEKLY_CLOSE,
    );
...
/** Best accuracy in the building, on observe()'s 0-100 scale. */
function staffQualityAccuracy(scouts: AttentionScout[]): number {
  return scouts.reduce((m, s) => Math.max(m, clamp(s.accuracy, 0, 100)), 40);
}
