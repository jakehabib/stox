...
export function Tooltip({ text, className = '', placement = 'top', align = 'center' }: {
  ...
  return (
    <span className={`relative inline-flex group ${className}`}>
      ...
      {/* The typography reset is load-bearing. Triggers hang off `.label-sm`
          (uppercase, letter-spaced, medium weight, often in a right-aligned
          cell) and `.section-title` (the bold condensed display face) — and a
          bubble nested inside one inherits the lot. Six lines of letter-spaced
          condensed capitals is not a sentence anybody reads. */}
      ...
      <span
        ...
        className={`pointer-events-none absolute z-50 w-72 rounded-md border border-line
                   ...
                   font-sans normal-case tracking-normal font-normal text-left
