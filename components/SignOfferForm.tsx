 ...
 *
 * What this component still owns is what is left AROUND the negotiation: the
 * Dynasty Market Knowledge band, and the real cap structuring —
 * front/back-loading and void years — that signing an outside free agent
 * shares with extending your own player. Those are cap decisions on your side
 * of the table; he does not judge them, so they move the ledger and never the
 * meter.
 *
 ...
 *
 * NEITHER IS THE RIVAL. This form used to assemble its own answer to "who else
 * is bidding" — a line reading "No other teams appear to be bidding on him
 * right now" when there was nobody, and `SuitorRumour` when there was. Both
 * are the panel's now, on all three tables, because the panel has the session
 * and the session already knows whether anybody may sign him today. The line
 * was also the weaker of the two claims: SuitorRumour's empty state says WHY
 * nobody is calling — the clubs with a hole at his position have no room or
 * better men to spend it on — which is the same fact with its evidence
 * attached. What is left below is the one thing this screen knows and the
 * session does not: what the GM's own scouting department has bought him.
 */
    ...
    <NegotiationPanel
      ...
      banner={
        /* The public market estimate is priced off what you can SEE of him;
           this is your staff's read on what he will actually put his name to.
           It is a band, and it is not centred perfectly — the skill informs,
           it does not solve. Nothing else is passed: a banner is for evidence
           this screen has and the session does not. */
        estimate ? (
