...
14 (App Router, TypeScript)**, **Tailwind CSS**, and **Prisma + SQLite**.
...
npm run db:push     # creates prisma/dev.db from the schema (SQLite, zero setup)
npm run dev          # http://localhost:3001
```

That's it — no API keys, no external services. `.env` already points at a
local SQLite file (`prisma/dev.db`), which is git-ignored so every clone
starts fresh. If you ever want to wipe your league and start over:

```bash
npm run db:reset     # deletes prisma/dev.db and recreates the schema
```
